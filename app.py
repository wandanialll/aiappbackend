...
def convert_audio_to_wav(input_path, output_path):
    ...
    try:
        # Try to load the audio file (pydub will auto-detect format)
        audio = AudioSegment.from_file(input_path)
        
        # Export as WAV with specific settings
        audio.export(
            output_path, 
            format="wav",
...
def transcribe():
    ...
    if not allowed_file(file.filename):
        return jsonify({"error": "Only audio files (WAV, MP3, OGG, WebM, M4A, FLAC) are allowed"}), 400
    ...
    try:
        ...
        if not filename.lower().endswith('.wav'):
            wav_file_path = os.path.join(app.config['UPLOAD_FOLDER'], f"{file_id}_converted.wav")
            if not convert_audio_to_wav(file_path, wav_file_path):
                return jsonify({"error": "Failed to convert audio to WAV format"}), 500
            os.remove(file_path)
            file_path = wav_file_path
        else:
            # Re-encode WAV files to ensure proper format
            temp_wav_path = os.path.join(app.config['UPLOAD_FOLDER'], f"{file_id}_temp.wav")
            if convert_audio_to_wav(file_path, temp_wav_path):
                os.remove(file_path)
                os.rename(temp_wav_path, file_path)
            else:
                logger.warning("Could not re-encode WAV file, proceeding with original")
...
def process():
    ...
    try:
        ...
        if not filename.lower().endswith('.wav'):
            ...
            # Even for .wav files, try to re-encode to ensure proper format
            temp_wav_path = os.path.join(app.config['UPLOAD_FOLDER'], f"{file_id}_temp.wav")
            if convert_audio_to_wav(file_path, temp_wav_path):
                os.remove(file_path)
                os.rename(temp_wav_path, file_path)
